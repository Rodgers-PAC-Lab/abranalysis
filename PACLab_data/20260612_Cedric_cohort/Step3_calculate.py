...
## Change our df to match ABRA formatting
# Remove the 40 pre-click samples
abra_df = big_abrs.loc[:, 0:].copy()
#Rename sound level and add fake frequency
...
if PLOT_FLIPPED_WAVEFORMS:
    for mouse in pre_HL.index.get_level_values('mouse').unique():
        ...
        for (channel ,speaker_side),subdf in gobj:
            ...
            ax.set_title(channel[::-1] + ' ' + speaker_side)
            my.plot.despine(ax)
            i_ax = i_ax + 1
        ...
        #Save
        savename = 'ABRA_pks_' + mouse + '_FLIPPED'
        f.suptitle(mouse + ' flipped/inverted')
