...
CNN_filepath = '/home/rowan/dev/abranalysis/utils/models/waveI_cnn.pth'
def interpolate_and_smooth(y, target_length=244):
    ...
    if len(y) == target_length:
        ...
        final = pandas.Series(interpolated_values)
    elif len(y) < target_length:
        cs = CubicSpline(x, y)
        final = cs(new_x)

    return pandas.Series(final)
...
def calculate_hearing_threshold(df, freq):
    ...
    for db in db_levels:
        ...
        if not khz.empty:
            ...
            final = pandas.to_numeric(final, errors='coerce')
...
def display_threshold_table(selected_dfs, selected_files, freqs):
    ...
    metrics_table = pandas.DataFrame(metrics_data)
...
def apply_manual_peak_edits(file_name, freq, db, x_values, y_values, highest_peaks, relevant_troughs):
    ...
    for column, latency_ms in manual_edits.items():
        if 'Peak' in column and 'latency' in column and not pandas.isna(latency_ms):
        ...
        elif 'Trough' in column and 'latency' in column and not pandas.isna(latency_ms):
...
def calculate_and_plot_wave(df, freq, db, peak_finding_model=default_peak_finding_model(filepath=CNN_filepath), return_peaks=True):
    file_name = getattr(df, 'name', 'unknown_file')
...
def calculate_and_plot_wave_exact(df, freq, db,
    ...
    if not khz.empty:
        ...
        tenms = int((10 / time_scale) * len(orig_y)) if time_scale > 10 else len(
            orig_y)
        ...
        if len(highest_peaks) > 0:
            ...
            relevant_troughs = np.array(
                [np.argmin(orig_y[trough - 1:trough + 2]) + trough - 1 for trough in relevant_troughs])
